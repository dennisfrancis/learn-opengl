...
int main()
{
    ...
    // clang-format off
    // note that we changed them to 'zoom in' on our texture image.
    float tex_min = 0.44f, tex_max = 0.55f;
    float vertices[] = {
        // counter clock wise.
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        int wrap_opt = (i == 0 ? GL_CLAMP_TO_EDGE : GL_REPEAT);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap_opt); // GL_REPEAT is the default.
