...
int main()
{
    ...
    util::Matrix4f scale_mat("scale_mat");

    // Setup shaders and program.
    util::Shader shader_program("shaders/vertex.vert", "shaders/fragment.frag", { &scale_mat });
...

    {
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if (scale > 1.5f || scale < 0.2f)
            {
                delta = -delta;
            }
            scale += delta;
            // set the uniform variable 4x4 matrix.
            // Here a uniform scaling is done.
